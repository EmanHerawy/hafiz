...
contract HafizDAO is IHafizDAO {
   using SendAPI for CommonTypes.FilActorId;
   ...
   uint256 public gracePeriod;
    ...
    HafizEjaza public membershipNFT;
     constructor(address[] memory _admins, address _membershipNFT, uint256 _gracePeriod) {
 ...
 function propose(
        ...
        proposals[proposalId] = EjazaProposal(msg.sender,to, new uint64[](0), cid,pieceCid,size, rewaya, parentCertId, block.timestamp, ProposalState.Proposed);
    ...
    function getActiveProviders(bytes32 proposalId) public view returns (uint64[] memory) {
     ...
     function getDataProof(bytes32 proposalId) external view  returns (string memory) {
        ...
        string memory pieceCid =string(proposals[proposalId].pieceCid);      
