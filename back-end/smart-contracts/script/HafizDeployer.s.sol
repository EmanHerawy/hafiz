...
contract HafizDeployer is Script {
    function run(address shikh1 ,uint256 _gracePeriod, address lighthouse) external {
...
//         HafizDAO dataDAO = new HafizDAO(admins, address(ejaza),_gracePeriod,lighthouse);
//         ejaza.setDAO(address(dataDAO));
//         HafizMarketplace marketplace = new HafizMarketplace(address(dataDAO), address(ejaza),3);
//         console.log(
...
//         // IEjaza.Recitations rewaya = IEjaza.Recitations.Hafs_3an_Aasem;
        ...
        //   dataDAO.joinDAO(); 
        // bytes32 proposalId = dataDAO.propose( cid, admins[0], parentCertId,  rewaya);
