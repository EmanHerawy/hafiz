...
contract TestFullScenario is Test {
 ...
 address lighthouse;
   ...
   // define address array for admins 
    ...
    dataDAO = new HafizDAO(admins, address(ejaza),_gracePeriod,lighthouse);
    ...
    function testCreateProposal() public{
      ...
      /**    bytes memory pieceCid,
        ...
        vm.expectRevert();
        bytes32 proposalId = dataDAO.propose( cid, Bob, parentCertId,  rewaya);
         ...
         vm.startPrank(Bob);
          dataDAO.joinDAO(); 
            proposalId = dataDAO.propose( cid, Bob, parentCertId,  rewaya);
            vm.stopPrank();
             vm.warp(1680616584 + 300 days);
             // execute
             vm.startPrank(Bob);
             dataDAO.executeProposal(proposalId);
                vm.stopPrank();
